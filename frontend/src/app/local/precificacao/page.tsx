...
import { Calculator, CheckCircle2, Filter, Printer, RefreshCw, Save, Search, Tags, TriangleAlert } from "lucide-react";
                        ...
                        <table className="w-full min-w-[950px] text-left print-table">
                            ...
                            <tbody className="divide-y divide-white/5">
                                {displayed.map(product => {
                                    ...
                                    return <tr key={product.sku} className="hover:bg-white/[0.02]"><td className="px-4 py-3"><p className="text-sm font-medium">{product.name}</p><p className="mt-0.5 font-mono text-[11px] text-slate-500">{product.sku}</p></td><td className={`px-3 py-3 text-right font-mono ${product.stock_available <= 0 ? "text-rose-300" : "text-slate-300"}`}>{product.stock_available}</td><td className="px-3 py-3 text-right font-mono text-slate-300">{money(product.calculated_cost)}</td><td className="px-3 py-3 text-right font-mono text-slate-300">{product.tax_rate_percent.toFixed(2)}%</td><td className="px-3 py-3 text-right font-mono text-slate-300">{product.target_margin_percent.toFixed(1)}%</td><td className="px-4 py-3 text-right"><span className="print:inline hidden font-bold">{money(product.selling_price)}</span><div className="no-print flex items-center justify-end gap-2"><span className="font-bold text-cyan-300">{money(product.selling_price)}</span><input value={editableValue} onChange={event => setEdits(current => ({ ...current, [product.sku]: event.target.value }))} className="w-20 rounded border border-slate-700 bg-black/20 px-2 py-1.5 text-xs outline-none focus:border-violet-500" /><button title="Salvar preço manual" onClick={() => void saveManualPrice(product)} className="rounded p-1.5 text-violet-300 hover:bg-violet-500/15"><Save className="w-4 h-4" /></button></div></td><td className="px-4 py-3 no-print">{product.status === "ready" ? <span className="inline-flex items-center gap-1 text-xs text-emerald-300"><CheckCircle2 className="w-3.5 h-3.5" /> {product.is_manual_price ? "Manual" : "Pronto"}</span> : <span title={product.warnings?.join(" ")} className="inline-flex items-center gap-1 text-xs text-amber-300"><TriangleAlert className="w-3.5 h-3.5" /> {product.status === "cost_pending" ? "Sem custo" : "Fiscal pendente"}</span>}</td></tr>;
                                })}
